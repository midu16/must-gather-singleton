...
def test_operator_info():
    ...
    assert operator_info(input_string) == {'operator_name': 'lvms-operator', 'operator_version': '4.12'}

def test_get_must_gather_url():
    info = {'operator_name': 'lvms-operator', 'operator_major_version': '4', 'operator_version': '4.12'}
    assert get_must_gather_url(info) == 'registry.redhat.io/lvms4/lvms-must-gather-rhel8:v4.12'

def test_invoke_must_gather():
    assert invoke_must_gather() == (True, "Success")
